...
describe("updater visual flow", () => {
  ...
  it("shows error state and detail bubble when GitHub API check fails", async () => {
    ...
    assert.deepStrictEqual(requests, [
      "api.github.com/repos/RRFRRF/clawd-on-desk4CodeWiz/releases/latest",
    ]);
  ...
  it("falls back to releases/latest redirect when GitHub API is rate-limited", async () => {
    ...
    const updater = initUpdater(ctx, makeDeps({
      httpsGetImpl: makeSequencedHttpsGet([
        ...
        },
        {
          statusCode: 302,
          headers: {
            location: "https://github.com/RRFRRF/clawd-on-desk4CodeWiz/releases/tag/v0.5.10",
          },
        },
      ], requests),
    }));
    ...
    assert.deepStrictEqual(requests, [
      "api.github.com/repos/RRFRRF/clawd-on-desk4CodeWiz/releases/latest",
      "github.com/RRFRRF/clawd-on-desk4CodeWiz/releases/latest",
    ]);
    assert.deepStrictEqual(bubbles.map((bubble) => bubble.mode), ["checking", "up-to-date"]);
  });
    ...
    const updater = initUpdater(ctx, makeDeps({
      ...
      httpsGetImpl: makeSequencedHttpsGet([
        ...
        },
        {
          statusCode: 302,
          headers: {
            location: "https://github.com/RRFRRF/clawd-on-desk4CodeWiz/releases/tag/v0.5.11",
          },
        },
      ], requests),
    }));
    ...
    const updater = initUpdater(ctx, makeDeps({
      ...
      httpsGetImpl: makeSequencedHttpsGet([
        ...
        },
        {
          statusCode: 302,
          headers: {
            location: "https://github.com/RRFRRF/clawd-on-desk4CodeWiz/releases/tag/v0.6.1",
          },
        },
      ], requests),
    }));
  ...
  it("uses the macOS packaged-update path by opening the releases page and showing a success bubble", async () => {
    ...
    try {
      ...
      assert.deepStrictEqual(bubbles.map((bubble) => bubble.mode), ["checking", "available", "ready"]);
      assert.strictEqual(openedUrls[0], "https://github.com/RRFRRF/clawd-on-desk4CodeWiz/releases/latest");
