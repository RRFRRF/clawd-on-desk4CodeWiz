...
### v0.15.100 Draft Smoke Checklist
...
- Confirm the packaged app shows `0.15.100` metadata.
...
- Download the native-package, Koffi prune/smoke, and updater metadata manifests.
  ...
  listed artifact filename identify `0.15.100`.
- For migration smoke, install v0.14.100 first and save a copy of the old
...
- Upgrade install over v0.14.100, launch, pet appears, no error dialog. Existing
  agent installation/enabled flags and user theme/animation choices remain intact.
- Settings -> About shows `v0.15.100`, sourced from `app.getVersion()`.
...
- Settings -> About contributors include the two v0.15.100 first-time
...
- Update labels never show a duplicated prefix such as `vv0.15.100`.
...
The legacy Telegram sidecar was removed in v0.14.100. Release builds must run
...
tag cut before this file landed can never trigger it — including v0.14.100, which
